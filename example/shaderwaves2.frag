...
float raymarchwater(vec3 camera, vec3 start, vec3 end, float depth)
{
    ...
    {
        float height =
            getwaves(pos.xz, ITERATIONS_RAYMARCH) * depth
            - depth;
...
vec3 normal(vec2 pos, float e, float depth)
{
    ...
    return normalize(
        cross(
            a - vec3(
                pos.x - e,
                getwaves(pos.xy - ex.xy, ITERATIONS_NORMAL) * depth,
                pos.y
            ),

            a - vec3(
                pos.x,
                getwaves(pos.xy + ex.yx, ITERATIONS_NORMAL) * depth,
                pos.y + e
            )
        )
    );
    ...
    return
        ...
        *
        createRotationMatrixAxisAngle(
            vec3(1.0, 0.0, 0.0),
            0.5 + 1.5 *
            (((NormalizedMouse.y == 0.0 ? 0.27 : NormalizedMouse.y))
            * 2.0 - 1.0)
        )
        *
        proj;
...
vec3 extra_cheap_atmosphere(vec3 raydir, vec3 sundir)
{
    ...
    vec3 suncolor = mix(
        ...
        max(
            vec3(0.0),
            vec3(1.0) - vec3(5.5,13.0,22.4) / 22.4
        ),
        special_trick2
    );

    vec3 bluesky =
        vec3(5.5,13.0,22.4) / 22.4
        * suncolor;
...
{
    return pow(
        max(0.0, dot(dir, getSunDirection())),
        720.0
    ) * 210.0;
}
